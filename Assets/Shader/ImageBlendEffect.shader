	...
	SubShader {
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.z = _EdgeSharpness - 1.0;
                tmp0.z = tmp0.z * 0.5;
                tmp0.w = _BlendAmount * 2.0 + -1.0;
                tmp1 = tex2D(_BlendTex, inp.texcoord.xy);
                tmp0.w = tmp0.w + tmp1.w;
                tmp1.w = saturate(tmp0.w * _EdgeSharpness + -tmp0.z);
                tmp0.xy = tmp0.xy * tmp1.ww;
                tmp0.xy = tmp0.xy * _Distortion.xx + inp.texcoord.xy;
                tmp0 = tex2D(_MainTex, tmp0.xy);
                tmp2.xyz = tmp1.xyz + float3(0.5, 0.5, 0.5);
                tmp2.xyz = tmp0.xyz * tmp2.xyz;
                tmp2.xyz = tmp2.xyz * float3(1.2, 1.2, 1.2) + -tmp1.xyz;
                tmp2.xyz = tmp2.xyz * float3(0.3, 0.3, 0.3);
                tmp2.w = 0.0;
                tmp1 = tmp1 + tmp2;
                tmp2.x = -tmp1.w * 0.5 + 1.0;
                tmp0.xyz = tmp0.xyz * tmp2.xxx;
                tmp2 = tmp1 - tmp0;
                o.sv_target = tmp1.wwww * tmp2 + tmp0;
                return o;
			}
			ENDCG
		}
