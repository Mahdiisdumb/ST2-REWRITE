	...
	SubShader {
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.w = tmp1.x * _HmdWarpParam.z + tmp0.w;
                tmp1.x = tmp1.x * _HmdWarpParam.w;
                tmp0.w = tmp1.x * tmp0.z + tmp0.w;
                tmp0.xy = tmp0.ww * tmp0.xy;
                tmp1 = tmp0.zzzz * tmp0.xyxy;
                tmp1 = tmp1 * _ChromaticAberration;
                tmp1 = tmp0.xyxy * _ChromaticAberration + tmp1;
                tmp0.xy = _Scale * tmp0.xy + _Center;
                tmp1 = _Scale.xyxy * tmp1 + _Center.xyxy;
                ...
                tmp0.x = tmp0.x != 0.0;
                tmp2.w = 1.0;
                o.sv_target = tmp0.xxxx ? float4(0.0, 0.0, 0.0, 0.0) : tmp2;
                return o;
			}
			ENDCG
		}
